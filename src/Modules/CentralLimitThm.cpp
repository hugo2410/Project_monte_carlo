...
CentralLimitThm::CentralLimitThm(AbstractVariable *pRandom,int multiples){
    std::string file_name = "../../src/output/sample_means_" + std::to_string(multiples*pRandom->get_size())+".csv";
...
void CentralLimitThm::verify_thm(AbstractVariable *pRandom,AbstractExpectation* pExpectation){
    ...
    std::string file_name = "../../src/output/sample_means_" + std::to_string(pRandom->get_size())+".csv";
